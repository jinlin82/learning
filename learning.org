...
** PDF 工具
   ...
   2. foxit reader
* Total Commander
  1. 在 explorer 中打开文件夹 ，I just use . (yes, that is a dot) or ctrl + enter enter
* DONE rime
1. https://www.zybuluo.com/eternity/note/81763
2. 11.0 以上版本与vim兼容性有问题，图标过大
3. 最后还是 11.0最满足要求，为了防止严重错误，必须注意以下事项：
   1. 注意：同步时不要同步 rime 文件夹，包括用户文件夹
   2. 安装在program files文件夹里面
   3. 删除上一次版本之前导出用户词典，并备份
   4. 清空用户文件夹
   5. 从备份文件中拷贝内容到3个文件，分别为
   default.custom.yaml
   weasel.custom.yaml
   # luna_pinyin_simp.custom.yaml 
   1. 把安装文件夹中的data/luna_pinyin_simp.schema.yaml 中的 ascii_mode    reset: 1
4. 注意：不要直接把 rime.bak 中所有的直接复制粘贴，否则会出现严重错误 
5. 导入用户词典，同步用户资料
6. 为了防止问题，可以先安装一次，卸载后再重新安装一次
...
*** 分支
    ...
    2. 分支创建
       - git branch testing
       - Magit操作: b n
       - Vs Code 操作: 点击左下角分支名称或者运行命令 git: create branch
    3. 列出所有分支(包括本地和远程)
       - git branch -a
       - Magit: Use magit-show-refs, bound by default to y. 或者在 status 中已经显示
       - Vs Code 操作: 点击左下角分支名称
    4. 切换分支
    ...
    5. 拉取(pull)远程分支
       - git fetch --all --prune
       - Magit: magit-fetch-all-prune
       - Vs Code 操作: 打开 Git Graph， 右上角 Fetch & Prune,然后状态栏右下角同步按钮
    6. 删除本地分支：切换到非当前要删除的分支上，再删除
       - git branch -d localBranchName
       - Magit: k 或者 b k
       - Vs Code 操作: git: branch delete 命令
    7. 删除远程分支：
       - git push origin --delete remoteBranchName（分支名称需要去掉origin，如果有）
       - Magit: k 或者 b k
       - 无，在终端中使用 git 命令
    8. 合并分支：git merge
    9. 常见工作场景
...
** Emacs + Git
   ...
   5. Emacs 中如果整个项目中含有中文文件夹，并且整个项目中没有比较长(跟具体中文有关系，一般达到8个中文字符没问题)的中文名文件的时候会出现两个问题：
      1. find-file-in-project 中文文件夹和中文文件名会乱码
      2. magit 中提交 commit 消息中文会乱码

      解决方法：
      - 在整个项目中添加一个中文名较长的文件， 如 .中文文件夹 文件，存在一个即可，可放在任意位置，一般放在中文文件夹下面，要push到项目中。
** git quick-stats
